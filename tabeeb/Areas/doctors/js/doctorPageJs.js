...
function addDoctorAdvice(adviceContent) {
    if (adviceContent.trim().length < 10 || adviceContent.trim() == "") {
        $('#adviceContent').val("");
        $('.adviceForm input').css('border-color', 'red');
        $('.adviceForm input').keydown(function () {
    ...
    $('.adviceForm button i').removeClass('fa-plus').addClass('fa-circle-o-notch fa-spin');
    $.post("/" + defaultPathForDoctorArea + "/addDoctorAdvice",
       { adviceContent: adviceContent },
       function (result, status) {
           if (result.status == true) {
               $('.adviceForm button i').addClass('fa-plus').removeClass('fa-circle-o-notch fa-spin');
               $('.adviceForm .alert-success').removeClass('hidden');
               $('.adviceForm input').val(language.enterValidAdvice);
               $('.adviceForm .alert-success').delay(4000).fadeOut(500, function () {
...
function updateDoctorAdvice(id, context) {
    ...
    $(context).removeClass('fa-edit').addClass('fa-circle-o-notch fa-spin');
    $.ajax({
        ...
        success: function (result) {
            if (result.status == true) {
                ...
                $(context).addClass('fa-edit').removeClass('fa-circle-o-notch fa-spin');
            }
            else {
                alert(operationProblem);
            }
...
function deleteDoctorAdvice(PK, context) {
    ...
    if (ans) {
        $(context).removeClass('fa-trash').addClass('fa-circle-o-notch fa-spin');
        $.ajax({
            ...
            success: function (result) {
                if (result.status == true) {
                    $(context).addClass('fa-trash').removeClass('fa-circle-o-notch fa-spin');
